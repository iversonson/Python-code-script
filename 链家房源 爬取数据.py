...
# https://gz.lianjia.com/ershoufang/pg+3+rs+%E8%B6%8A%E7%A7%80/
    ...
    query = pyquery.PyQuery(content)
    soup = bs4.BeautifulSoup(content,'lxml')
    rank_list = query('#content > div.leftContent > ul > li:nth-child(1) > div.info.clear > div.title > a')
    ...
    for i in range(item_num):
        print(titles[i].getText(),position[i].getText(),total_price[i].getText(),uniprice[i].getText())
